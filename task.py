...
def label_farthest_separate(sentence, y):
    farthest = (sentence @ y).argmin()
    return sentence[farthest, :]
...
class FarthestPointSeparateDataset(torch.utils.data.IterableDataset):
    ...
    def __iter__(self):
        ...
        while True:
            sentence = gen_sentence(self.ntokens, self.dim, rng=rng)
            # sentence = torch.concat((sentence, -sentence))
            y = gen_sentence(1, self.dim, rng=rng).squeeze()
            yield sentence, y, label_farthest_separate(sentence, y)
